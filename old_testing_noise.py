...
for i in range(1, 3):
    for j in range(1, 3):
        ...
        # version = '2_48'
        # version = 'noise_48_2'
        # version = 'res_noise_norm_4'#4
        version = '48_res_noise_norm_6'#4
        experiment = '8'
        size = (*[96, ] * 3 + [1],)
        lr = 1e-3
        ...
        ants.image_write(ants.reorient_image2(ants.from_numpy(out), 'LPI'),
                         os.path.join(input_path, f'output2_noise{version}-{experiment}.nii'))
        ...
        # image_pha *= TE
        image_pha /= (42.58 * 7)
        ants.image_write(ants.from_numpy(np.squeeze(image_pha)), os.path.join(input_path, 'input_phase.nii.gz'))
        image_pha = central_crop(image_pha, (160, 192, 192))
        image_pha *= image_mag
        ...
        ants.image_write(ants.reorient_image2(ants.from_numpy(out), 'LPI'),
                         os.path.join(input_path, f'output_noise{version}-{experiment}.nii'))


        image_pha, _ = open_nii_gz(os.path.join(input_path, 'input_phase.nii.gz'))
        ...
        ants.image_write(ants.reorient_image2(ants.from_numpy(out), 'LPI'),
                         os.path.join(input_path, f'output3_noise{version}-{experiment}.nii'))