...
for i in range(1, 3):
    for j in range(1, 3):
        ...
        input_path = f"challenge\\Sim{i}Snr{j}"
        image_pha, original_data = open_nii_gz(
            os.path.join(input_path, "Frequency.nii.gz")
        )
        image_mag, _ = open_nii_gz(
            os.path.join(input_path, "MaskBrainExtracted.nii.gz")
        )
        ...
        print(
            os.path.join(input_path, f"output_noise_{model_version}-{experiment}.nii")
        )
        ants.image_write(
            ants.reorient_image2(original_data.new_image_like(out), "LPI"),
            os.path.join(
                input_path,
                str(experiment),
        ...
        delta = np.squeeze(ground) - np.squeeze(out)
        new = max_side_padding(np.squeeze(out))
        GT = max_side_padding(np.squeeze(ground))
