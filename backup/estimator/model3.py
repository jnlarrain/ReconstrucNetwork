...
def multy_layer(inputs, number_filter):
    ...
    layer_a = Conv3D(number_filter // 4, kernel_size=(1, 1, 1), padding='same')(layer_a)
    layer_a = LeakyReLU()(layer_a)

    layer_b = Conv3D(number_filter // 4, kernel_size=(5, 5, 5), padding='same')(inputs)
    layer_b = LeakyReLU()(layer_b)
    layer_b = Conv3D(number_filter // 4, kernel_size=(1, 1, 1), padding='same')(layer_b)
    layer_b = LeakyReLU()(layer_b)

    layer_e = Conv3D(number_filter // 4, kernel_size=(1, 1, 1), padding='same')(inputs)
    layer_e = LeakyReLU()(layer_e)
    if number_filter == tamano:
        ...
        final = concatenate([layer_a, layer_b, layer_c, layer_d, layer_e], axis=4)

    else:
        final = concatenate([layer_a, layer_b, layer_e], axis=4)
...
class Network:
    def main(self, inputs):
        ...
        with tf.name_scope('layer9'):
            ...
            l9b = Conv3DTranspose(tamano, kernel_size=(2, 2, 2), padding='same')(l9a)
