from tensorflow.layers import conv3d, max_pooling3d, batch_normalization, dropout, conv3d_transpose, dense
...
def network(inputs, image_size, training=False):
    ...
    l1 = relu(l1)
    l1 = conv3d(l1, 16, kernel_size=(1, 1, 1), padding='same')  # /1
    l1 = batch_normalization(l1)
    l1 = relu(l1)
    ...
    l1 = relu(l1)
    # print(l1)
    # l1a = dropout(l1, .5, training)
    # l1a = dense(l1a, 512)
    # l1a = dropout(l1a, .5, training)

    # Layer 2
    # l2 = max_pooling3d(l1, 2, 2)  # /2
    l2 = conv3d(l1, 32, kernel_size=(5, 5, 5), padding='same', strides=2)  # /2
    ...
    l3 = conv3d(l2, 64, kernel_size=(5, 5, 5), padding='same', strides=2)  # /4
    l3 = batch_normalization(l3)
    l3 = relu(l3)
    l3 = conv3d(l3, 32, kernel_size=(1, 1, 1), padding='same')  # /4
    l3 = relu(l3)
    l3 = conv3d(l3, 64, kernel_size=(5, 5, 5), padding='same')  # /4
    l3 = batch_normalization(l3)
    l3 = relu(l3)
    # print(l3)
    # l1a = dropout(l1, .5, training)
    ...
    l4 = conv3d(l3, 64, kernel_size=(5, 5, 5), padding='same', strides=2)  # /8
    ...
    l5 = conv3d(l4, 128, kernel_size=(5, 5, 5), padding='same', strides=2)  # /16
