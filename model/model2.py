...
def extended_context_stride_2_convolution(inputs, number_filter):
    dilatation_convolutional_layer = Conv3D(
        number_filter // 2, kernel_size=(3, 3, 3), dilation_rate=2, padding="same"
    )(inputs)
    activation_layer = LeakyReLU()(dilatation_convolutional_layer)
    dimentional_reduction_layer = Conv3D(number_filter // 4, kernel_size=(1, 1, 1), padding="same")(activation_layer)
    activation_layer = LeakyReLU()(dimentional_reduction_layer)
    stride_2_convolutional_layer = Conv3D(number_filter // 2, kernel_size=(3, 3, 3), strides=2, padding="same")(
        activation_layer
    )
...
def composed_convolutional_block(inputs, number_filter):
    return BatchNormalization()(
        tf.concat(
            [
                normal_context_stride_2_convolution(inputs, number_filter),
                extended_context_stride_2_convolution(inputs, number_filter),
            ],
            -1,
        )
    )
...
def convolutional_dense(inputs, number_filter):
    ...
    layer_a = LeakyReLU()(layer_a)
    layer_a = Conv3D(number_filter // 2, kernel_size=(3, 3, 3), padding="same")(layer_a)
    layer_a = LeakyReLU()(layer_a)
